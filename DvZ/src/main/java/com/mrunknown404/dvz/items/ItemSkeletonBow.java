...
public class ItemSkeletonBow extends ItemBase {
	...
	private void craftArrows(World world, EntityPlayer player, ItemStack stack) {
		world.playSound(player, player.getPosition(), SoundEvents.ENTITY_ARROW_HIT_PLAYER, SoundCategory.HOSTILE, 1.0f, 1.0f);
		if (!world.isRemote) {
		...
		if (entityLiving instanceof EntityPlayer) {
			...
			if (!itemstack.isEmpty() || flag) {
				...
				if ((double)f >= 0.1D) {
					...
					world.playSound(entityplayer, entityplayer.getPosition(), SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.HOSTILE, 1.0F, 1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + f * 0.5F);
